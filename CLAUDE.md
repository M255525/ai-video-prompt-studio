...
**「AI 優化結果」自動漏掉鏡頭運動／風格／光線／氛圍的修法（2026-09-19，比照 `ai-image-prompt-studio` 已驗證過的踩坑修法搬過來）**：原本 `aiPromptForTarget()` 讓 AI 自己從一串「指定鏡頭運動／風格／光線／氛圍（請自然融入）：A、B、C」的**列表**重新組織成 1～4 句自然語言，AI 很自然會選擇性省略「看起來不影響主要畫面」的項目，且「中文說明」原本只要求「簡短描述你理解的畫面內容」，完全不要求涵蓋這些標籤，導致 AI 優化結果的中英文都常常漏掉風格/構圖(鏡頭)/光線氛圍。改法：
  - `assembleForTarget()` 拆成 `assembleCoreLines(target, v)`（純組裝核心陣列，不含給人類看的頁尾提醒）＋ `assembleForTarget()`（在核心上加頁尾提醒）。沒有既有提示詞補充時，`aiPromptForTarget()` 不再讓 AI 自己從欄位重新組裝，而是直接把「🔧 組成提示詞」用的同一份 100% 完整的規則式草稿（`assembleCoreLines()`）交給 AI 翻譯潤飾——AI 的任務從「記得列出每一項設定」降低成「把已經列好的草稿翻成通順文字」，大幅降低遺漏風險。`hasExisting`（既有腳本補充）分支不受影響，仍走條列句子提供上下文（無對應草稿可翻譯），但補上「每一項都要有對應內容、不可為了精簡而省略」的措辭。
  - 新增共用函式 `mandatoryEnglishTrailerRule(v, ratioOpt)`／`mandatoryChineseTrailerRule(v, ratioOpt)`，要求最終輸出（English Prompt／可靈中文提示詞／中文說明）結尾**必須**依序另外加上固定格式短句（`"Duration: 8 seconds."`／`"Aspect ratio: 1:1."`／`"Audio: ..."`／`"Avoid: ..."` 或對應中文版），把時長／長寬比／音效配樂／不想出現的元素變成獨立於描述句之外、不需要 AI 自行判斷要不要犧牲的必填項目。`aiPromptForTarget()` 的四個分頁 `formatRule` 都串接對應的 trailer 函式；`kling` 分頁的「中文說明」「可靈中文提示詞」「English Prompt (optional)」三段都各自附上。
  - 「中文說明」的措辭從「簡短描述你理解的畫面內容」改成「完整度必須跟後面的正式輸出對等，不是一句話摘要，要把鏡頭運動／風格／光線／氛圍也具體寫進去」；結尾新增總結句「中文說明與 English Prompt（或「可靈中文提示詞」）都必須完整涵蓋草稿裡的每一項設定，不可為了精簡而省略」。
  - 已用 Playwright 監聽 `window.fetch` 攔截實際送給 `callLLM()` 的完整 prompt 文字（繞過 `#licenseGate`、填假金鑰、讓 fetch 直接 reject 避免真的打 API），套用範例後對四個分頁分別驗證：Sora／Veo／Runway 三分頁的中文說明與 English Prompt trailer 皆正確帶入時長/長寬比/音效/避免元素的固定寫法；Kling 分頁的三段輸出（中文說明／可靈中文提示詞／English Prompt）皆正確帶入。
  - **同日第二輪補強（使用者用真實 API 金鑰實測 Veo 分頁回報「中文與英文不相同，中文少了 Camera／Style／Lighting／Ambiance／Duration／長寬比／Audio」）**：第一輪只把「鏡頭運動／風格／光線／氛圍」放進中文說明的**軟性要求**（「請把…也具體寫進這段中文說明裡」），沒有像時長／長寬比／音效／避免元素那樣變成固定格式的**必填短句**——因為 Veo 分頁的 English Prompt 有 `formatRule` 本身強制要求 Camera／Style／Ambiance 各自成行，英文結果完整，但中文說明沒有對應的強制結構，AI 傾向寫成一段流暢短文、不含這些標籤內容，導致中英兩段實測明顯不對等。修法：`mandatoryChineseTrailerRule(v, ratioOpt)` 擴充成也包含鏡頭運動／風格／光線／氛圍（比照時長／長寬比／音效／避免的固定格式「「標籤：內容。」」逐行列出，取自 `optZh(CAMERA_OPTIONS, ...)` 等同一組欄位值，不需要 AI 自己翻譯判斷），中文說明的指示也從「請把…具體寫進這段中文說明裡」改成「先用 2～3 句話流暢描述主體、動作與場景，寫完敘述句之後，另外換行逐項列出這幾行固定寫法」——中文說明現在變成「流暢開場句＋逐項固定格式標籤」兩段式，跟英文（Veo 的 Subject/Context/Action/Camera/Style/Ambiance 結構，或其他分頁的敘述句+trailer）在「每一項設定都有明確可核對的對應內容」這一點上完全對等。已用 Playwright 重新截取 Veo 分頁（沿用使用者實測時用的「街頭咖啡師」範例）送給 AI 的完整 prompt，確認「鏡頭運動：緩慢推近。」「風格：電影感寫實。」「光線：自然光。」「氛圍：寧靜。」「時長：約 8 秒。」「長寬比：9:16，…」「音效／配樂：…」七行固定寫法皆正確列在中文說明的指示中。

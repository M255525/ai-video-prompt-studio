/**
 * AI 短影片提示詞產生器 — 授權序號驗證後端
 *
 * 部署到綁定 Google Sheet 的「擴充功能 > Apps Script」，部署為 Web App 後，
 * 把取得的網址填進 index.html 的 LICENSE_CHECK_URL（序號授權閘門 IIFE 內）。
 * 完整部署步驟見 SETUP-授權伺服器設定.md。
 *
 * 對應的 Google Sheet 需有「序號」「開始日期」「結束日期」這三個表頭欄位
 * （欄位順序不拘、可以跟其他既有欄位混在一起，程式是用表頭文字比對欄位，不是靠欄位順序）。
 * 開一個新的授權對象時：在表格新增一列，填「序號」欄，「開始日期」「結束日期」留空
 * ——序號第一次被驗證時會自動寫入（開始日期＝當下時間，結束日期＝開始日期 + 12 個月）。
 */
